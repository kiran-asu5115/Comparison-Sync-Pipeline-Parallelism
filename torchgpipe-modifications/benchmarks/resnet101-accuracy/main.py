...
class Experiments:
    ...
    def gpipe256(model: nn.Module, devices: List[int]) -> Stuffs:
        batch_size = 256
        chunks = 4
        # balance = [140, 230]  # For ResNet-101
        balance = [73, 110]  # For ResNet-50
...
def dataloaders(batch_size: int, num_workers: int = 32) -> Tuple[DataLoader, DataLoader]:
    ...
    return train_dataloader, test_dataloader, len(class_names)
...
def cli(ctx: click.Context,
    ...
    model: nn.Module = resnet50(num_classes=num_classes, inplace=relu_inplace)
    try:
    ...
    return
    # Prepare dataloaders.
    train_dataloader, valid_dataloader, _ = dataloaders(batch_size)
